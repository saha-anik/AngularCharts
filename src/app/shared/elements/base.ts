...
export class StatsBase<T extends IStatsBaseProps, O extends IStatsBaseOptions> extends Element<T, O> {
  ...
  _getBounds(_useFinalPosition?: boolean): TooltipArea[] {
    // abstract
    return [
      {
        left: 0,
  ...
  _getHitBounds(useFinalPosition?: boolean): TooltipArea[] {
  ...
  inXRange(mouseX: number, useFinalPosition?: boolean): boolean {
    ...
    for (let i = 0; i < bounds.length; i++) {
      if (mouseX >= bounds[i].left && mouseX <= bounds[i].right) return true;
    }
    return false;
  }
  ...
  inYRange(mouseY: number, useFinalPosition?: boolean): boolean {
    const bounds = this._getHitBounds(useFinalPosition);
    for (let i = 0; i < bounds.length; i++) {
      if (mouseY >= bounds[i].top && mouseY <= bounds[i].bottom) return true;
    }
    return false;
  }
  ...
  protected _boxInRange(mouseX: number, mouseY: number, useFinalPosition?: boolean): boolean {
    ...
    for (let i = 0; i < bounds.length; i++) {
      if (
        mouseX >= bounds[i].left &&
        mouseX <= bounds[i].right &&
        mouseY >= bounds[i].top &&
        mouseY <= bounds[i].bottom
      )
        return true;
    }
    return false;
